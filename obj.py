...
class ObjectController(Controller):
    ...
    def GET(self, req):
        ...
        with open('/tmp/log.txt','a+') as f:
            if 'swift_conn' in dir(resp):
                file_name = str(resp.headers['X-Timestamp'])
                f.write('path: ' +'%r'%(resp.swift_conn.path) +'\n')
                folder_temp = str(resp.swift_conn.path).split('/')[3:]
                from hashlib import md5
                folder_hash = md5('/' + '/'.join(folder_temp) + 'fLIbertYgibbitZ').hexdigest()                
                f.write('hash_folder :' + folder_hash +'\n')
                f.write('host: ' +'%r'%(resp.swift_conn.host) +'\n')
                partition_id = str(resp.swift_conn.path).split('/')[2]
                #final_path = 'http://'+ str(resp.swift_conn.host)+'/' + partition_id + '/' + folder_hash[-3:]+ '/' + folder_hash + '/'+str(resp.swift_conn.path).split('/')[-1]
                final_path = 'http://'+ str(resp.swift_conn.host)+'/' + partition_id + '/' + folder_hash[-3:]+ '/' + folder_hash + '/'+ file_name + '.data'
                f.write('final path :'+ final_path +'\n')
                #r = redis.Redis(unix_socket_path='/tmp/redis.sock')
                pool = redis.ConnectionPool(host='127.0.0.1', port=6379, db=0)
                r = redis.Redis(connection_pool=pool)
                r.setex("/".join(resp.swift_conn.path.split('/')[5:]),final_path, 60)
        return resp
